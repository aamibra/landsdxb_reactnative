...
import { e, evaluateunitform_api, getunitform_api } from '@/constant/DXBConstant';
...
export default function UnitPolicy({ navigation, route }: any) {
  ...
  useEffect(() => {
    const loadPolicy = async () => {
      ...
      try {
        const res = await api.get(getunitform_api, {
          params: { formid },
        });
    ...
    <View style={{ flex: 1 }}>
      <PagerView style={styles.pagerView} initialPage={0} ref={pagerRef}>
        ...
        <ScrollView contentContainerStyle={styles.page} key="2">
          <RTLText style={styles.header}>{i18n.t('evaluationunit')}</RTLText>
