...
import { creatunderconstpolicy_api, e, getunderconstpolicy_api, updateunderconstpolicy_api } from '@/constant/DXBConstant';
...
export default function UnderConstPolicy({ navigation, route }: any) {
  ...
  useEffect(() => {
    const loadPolicy = async () => {
      ...
      try {
        const res = await api.get(getunderconstpolicy_api, {
          params: { policyid },
        });
        ...
        if (res.status === 200 && res.data) {
          ...
          setFormData({
            ...
            percentcompletion: data.percentcompletion || '0',
            ...
            buildareapricesqm: '',
            buildareagrossvalue: data.buildareagrossvalue || '0',
            ...
            Consltletter: null,
            consultantletterid: data.otherdocumentid || '',
            consultantlettername: data.otherdocumentname || '',
            consultantletterimg: data.otherdocumentimg || '',
          });
  ...
  useEffect(() => {
    calculatePercentGrossValue();
  }, [
    formData.percentcompletion,
    formData.buildareagrossvalue
  ]);
  ...
  const pickDocument = async (key) => {
    ...
    if (!result.canceled && result.assets.length > 0) {
      ...
      } else if (key === 'Consltletter') {
        handleChange('consultantlettername', fileToUpload.name);
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      ServerData.append("percentcompletion", formData.percentcompletion);
      ...
      if (formData.Consltletter && formData.Consltletter.uri) {
        ServerData.append("consultantletterfile", formData.Consltletter);
      }
      ...
      if (formData.consultantletterid) {
        ServerData.append("consultantletterid", formData.consultantletterid);
      }
      ...
      const response = await fetch(
        isEdit ? updateunderconstpolicy_api : creatunderconstpolicy_api,
        {
          method: 'POST',
          body: ServerData,
          headers: {
  ...
  const openPreview = () => {
    ...
    // Consult Letter
    if (formData.Consltletter?.uri) {
      files.push(prepareFile(formData.Consltletter.uri, formData.Consltletter.type, formData.Consltletter.name, i18n.t('consultletter')));
    } else if (formData.consultantletterimg) {
      const base64 = formData.consultantletterimg;
      const isPdf = base64.startsWith('%PDF') || base64.includes('application/pdf');
      const type = isPdf ? 'application/pdf' : 'image/jpeg';
      const uri = `${base64}`;
      files.push(prepareFile(uri, type, formData.consultantlettername || 'Consult_Letter', i18n.t('consultletter')));
    }
  ...
  const calculatePercentGrossValue = () => {
    const percentcompRaw = formData.percentcompletion;
    const grossbuildRaw = formData.buildareagrossvalue;

    const percentcomp = parseFloat(percentcompRaw) / 100 || 0;
    const grossbuild = cleanNumber(grossbuildRaw);

    if (grossbuild && grossbuild !== 0) {
      const totalgross = grossbuild * percentcomp;
      const rounded = parseFloat(totalgross.toFixed(2));
      setGrossValue(numberCommas(rounded));
    } else {
      setGrossValue('0.00');
    }
  };
  ...
  const calculateCostRealEstate = (
    landareagrossvalue: number,
    buildareagrossvalue: number,
    percentcost: number,
    ...
    if (
      ...
      let _compgrossbuild, sumlandbuild, multilandbuild;

      if (buildareagrossvalue && buildareagrossvalue !== 0) {
        const percentFactor = percentcompletion / 100;

        _compgrossbuild = buildareagrossvalue * percentFactor;
        // , totalgross
        // totalgross = percentFactor * buildareagrossvalue;
        sumlandbuild = landareagrossvalue + _compgrossbuild;

        if (percentcost && percentcost !== 0) {
          const percentGrossFactor = percentcost / 100;
          multilandbuild = _compgrossbuild * percentGrossFactor;
          sumlandbuild += multilandbuild;
        }

        return numberCommas((sumlandbuild).toFixed(2));
      }
    }
  ...
  const handleSmartChange = (field: any, value: any) => {
    ...
    setFormData(prev => {
      ...
      updated.costrealestate = calculateCostRealEstate(
        ...
        cleanNumber(updated.percentcompletion),
      );
    ...
    <View style={{ flex: 1 }}>
      <PagerView style={styles.pagerView} initialPage={0} ref={pagerRef}>
        ...
        <ScrollView contentContainerStyle={styles.page} key="1">
          <RTLText style={styles.header}>{i18n.t('underconstform')}</RTLText>
          ...
          >
            ...
            <View style={styles.pickerWrapper} >
              ...
              >
                ...
                {Object.entries(dropdown)
                  .filter(([key]) => key === 'underconstevatype')
                  .map(([menuKey, options]) => (
                    options.map((item) => (
                      <Picker.Item key={item.id} label={isArabic ? item.name_ar : item.name_en} value={item.id} />
                    ))
          ...
          {/* معلومات قيد البناء */}
          <AccordionSection
            title={i18n.t('underconstinformation')}
            isCollapsed={collapse.underconst}
            toggle={() => toggleSection('underconst')}
          >
            ...
            <RTLText style={styles.label}>{i18n.t('percentagecompletion')}</RTLText>
            <View style={styles.custominput} >
              <PercentageInput value={formData.percentcompletion} onChange={(text: any) => handleChange('percentcompletion', text)} />
            </View>
          </AccordionSection>
          ...
          >
            ...
            <RTLText style={styles.label}>{i18n.t('consultletter')}</RTLText>
            <TouchableOpacity style={styles.uploadButton} onPress={() => pickDocument('Consltletter')}>
              <RTLText>{formData.consultantlettername || (formData.Consltletter ? i18n.t('consultletteruploaded') : i18n.t('uploadconsultletter'))}</RTLText>
            </TouchableOpacity>
        ...
        <ScrollView contentContainerStyle={styles.page} key="2">
          <RTLText style={styles.header}>{i18n.t('underconstevaluation')} </RTLText>
          ...
          >
            ...
            <View style={styles.fieldsContainer}>
              <View style={styles.fieldRow}>
                <RTLText style={[styles.label, { marginRight: 5 }]} >{i18n.t('grossvaluebuild')}</RTLText>
                <TextInput
                  style={[styles.input, { marginLeft: 5 }]}
                  keyboardType="numeric"
                  value={formData.buildareagrossvalue}
                  editable={false}
                />
              </View>

              <View style={styles.fieldRow}>
                <RTLText style={[styles.label, { marginRight: 5 }]} >{i18n.t('percentagecompletion')}</RTLText>
                <TextInput
                  style={[styles.input, { marginLeft: 5 }]}
                  keyboardType="numeric"
              ...
              <View style={styles.fieldRow}>
                <RTLText style={[styles.label, { marginRight: 5 }]} >{i18n.t('grossvalue')}</RTLText>
                <TextInput
                  style={[styles.input, { marginLeft: 5 }]}
                  keyboardType="numeric"
                  value={grossValue}
