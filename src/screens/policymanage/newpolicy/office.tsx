...
import { creatofficepolicy_api, e, getofficepolicy_api, updateofficpolicy_api } from '@/constant/DXBConstant';
...
export default function OfficePolicy({ navigation, route }: any) {
  ...
  useEffect(() => {
    const loadPolicy = async () => {
      ...
      try {
        const res = await api.get(getofficepolicy_api, {
          params: { policyid },
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        isEdit ? updateofficpolicy_api : creatofficepolicy_api,
        {
          method: 'POST',
          body: ServerData,
          headers: {
    ...
    <View style={{ flex: 1 }}>
      <PagerView style={styles.pagerView} initialPage={0} ref={pagerRef}>
        ...
        <ScrollView contentContainerStyle={styles.page} key="1">
          ...
          {/* معلومات مكتب */}
          <AccordionSection
            title={i18n.t('officeinformation')}
