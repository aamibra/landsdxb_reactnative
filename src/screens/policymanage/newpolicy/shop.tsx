...
import { creatshoppolicy_api, e, getshoppolicy_api, updateshoppolicy_api } from '@/constant/DXBConstant';
...
export default function ShopPolicy({ navigation, route }: any) {
  ...
  useEffect(() => {
    const loadPolicy = async () => {
      ...
      try {
        const res = await api.get(getshoppolicy_api, {
          params: { policyid },
        });
        ...
        if (res.status === 200 && res.data) {
          ...
          setFormData(prev => {
            ...
            let expectedincome = data.expectedincome || '';
            let incomegrossery = data.incomegrossery || '';
            let rentvalue = data.rentvalue || '';
            let incomepercent = data.incomepercent || '0';
            ...
            expectedincome = numberWithCommas(expectedincome) || '';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        isEdit ? updateshoppolicy_api : creatshoppolicy_api,
        {
          method: 'POST',
          body: ServerData,
          headers: {
    ...
    <View style={{ flex: 1 }}>
      <PagerView style={styles.pagerView} initialPage={0} ref={pagerRef}>
        ...
        <ScrollView contentContainerStyle={styles.page} key="1">
          <RTLText style={styles.header}>{i18n.t('shopform')}</RTLText>
          ...
          {/* معلومات محل */}
          <AccordionSection
            title={i18n.t('shopinformation')}
            isCollapsed={collapse.shop}
            toggle={() => toggleSection('shop')}
          >
            <RTLText style={styles.label}>{i18n.t('shoptype')}</RTLText>
            <View style={styles.pickerWrapper}>
              <Picker
                style={{ height: 70 }}
              ...
              >
                <Picker.Item label={i18n.t('internalshop')} value="1" />
                <Picker.Item label={i18n.t('externalshop')} value="2" />
              </Picker>
            </View>
        ...
        <ScrollView contentContainerStyle={styles.page} key="2">
          <RTLText style={styles.header}>{i18n.t('evaluationshop')}</RTLText>
          ...
          >
            <SectionLabel title={i18n.t('priceshoparea')} />
            ...
            <SectionLabel title={i18n.t('valuedependincome')} />
            <RTLText style={styles.label}>{i18n.t('expectedincome')}</RTLText>
            <TextInput style={styles.input} placeholder={i18n.t('expectedincome')} value={formData.expectedincome} onChangeText={(text) => handleSmartChange('expectedincome', text)} keyboardType="numeric" />
            ...
            <RTLText style={styles.label}>{i18n.t('grossvalue')}</RTLText>
            <TextInput style={styles.input} placeholder={i18n.t('grossvalue')} value={formData.incomegrossery} onChangeText={(text) => handleChange('incomegrossery', text)} keyboardType="numeric" />
