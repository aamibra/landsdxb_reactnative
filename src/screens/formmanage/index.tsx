...
const FormScreen = ({ navigation }: any) => {
  ...
  const fetchForms = async (reset = false) => {
    ...
    try {
      ...
      // Alert.alert(i18n.t('error'), i18n.t('failedtofetchforms'));
  ...
  const handleFormOption = () => {
    ...
    options.push(i18n.t('cancel')); // Cancel button label
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ====== زر عائم لإضافة نموذج ====== */}
      <TouchableOpacity
        style={styles.fab}
        onPress={() => formRef.current?.openActionSheet()}
        activeOpacity={0.8}
      >
      ...
      {/* ====== نافذة اختيار ====== */}
      <FormSelector ref={formRef} navigation={navigation} />
    </View>
  );
};
